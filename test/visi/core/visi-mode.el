...
;; 2015-01-21
;; first version
;; open a visi file (e.g. sample_visi_code.visi)
;; call visi-mode
;; call cider-jack-in
;; call cider-eval-region on the following lin
...
You must call `cider-jack-in' first.
To eval clojure code, call `cider-eval-last-sexp', `cider-eval-region' etc."
  ...
  (if (cider-connected-p)
      (progn
        (let ((namespace (visi-gen-random-namespace )))
          (cider-interactive-eval
           (format "(vp/parse-and-eval-for-tests \"%s\")"
                   (buffer-substring-no-properties p1 p2)) p1)))
    (progn (error "No active nREPL connection. Call `cider-jack-in' or `cider-connect' first."))))
